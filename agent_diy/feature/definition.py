    ...
    def compute_returns(self, last_values, gamma, lam):
        ...
        for step in reversed(range(self.num_transitions_per_env)):
            if step == self.num_transitions_per_env - 1:
                # 对于最后一步，其"下一步"的价值由函数外部传入(last_values)
                next_values = last_values
            else:
                # 对于其他步，其"下一步"的价值就是存储中的后一步的价值
                next_values = self.values[step + 1]
            # 如果"下一步"不是终止状态，next_is_not_terminal为1.0，否则为0.0
            next_is_not_terminal = 1.0 - self.dones[step].float()
            # 计算TD-error (时序差分误差): δ = r + γ * V(s') - V(s)
            delta = (
                self.rewards[step]
                + next_is_not_terminal * gamma * next_values
                - self.values[step]
            )
            # GAE递推公式: A(s,a) = δ + γ * λ * A(s',a')
            advantage = delta + next_is_not_terminal * gamma * lam * advantage
            # 计算回报(Return)，回报 = 优势 + 价值
            self.returns[step] = advantage + self.values[step]

        # 计算完所有步的原始优势后，进行标准化处理
        self.advantages = self.returns - self.values
        # 将优势函数标准化为均值为0，方差为1的分布
