...
def _reward_a(self) -> torch.Tensor:
    global init, real_env_origins, last_reset_buf
    if not init:
        init = True
        ...
        # 第一次起始位置
        real_env_origins = self.env_origins[:, :2].clone()
        last_reset_buf = torch.zeros(
            self.num_envs, dtype=torch.bool, device=self.device, requires_grad=False
        )

    # 更新被重置 env 的起始位置
    env_ids = last_reset_buf.nonzero(as_tuple=False).flatten()
    if len(env_ids) > 0:
        real_env_origins[env_ids] = self.env_origins[env_ids, :2]

    # 用root_states替换env_origins,阻止距离检测触发
    self.env_origins[:, :2] = self.root_states[:, :2]
    ...
    last_reset_buf.copy_(self.reset_buf)

    # 还原需要重置的 env 的起始位置
    env_ids = self.reset_buf.nonzero(as_tuple=False).flatten()
    if len(env_ids) > 0:
        self.env_origins[env_ids, :2] = real_env_origins[env_ids]
